...
//contador para a menoor solucao
...
// matriz referente aos caminhos 0eu posso ir  1 é parede
...
int resolve(char m[][N], int i, int j, Pontos *p) {
    ...
    if (i + 1 < N && mat[i + 1][j] != 1 && !jaEstive(p - 1, i + 1, j) && achei == 0 && possibilidades < 64) {
        ...
        p->j = j;
        possibilidades++;
        resolve(m, i + 1, j, p + 1);//tentar descer
    }
    if (i - 1 >= 0 && mat[i - 1][j] != 1 && !jaEstive(p - 1, i - 1, j) && achei == 0 && possibilidades < 64) {
        p->i = i;
        p->j = j;
        possibilidades++;
        resolve(m, i - 1, j, p + 1);//tentar subir
    }
    ...
    if (j + 1 < N && mat[i][j + 1] != 1 && !jaEstive(p - 1, i, j + 1) && achei == 0 && possibilidades < 64) {
        ...
        p->j = j;
        possibilidades++;
        resolve(m, i, j + 1, p + 1);//tentar ir para direita
    }
    if (j - 1 >= 0 && mat[i][j - 1] != 1 && !jaEstive(p - 1, i, j - 1) && achei == 0 && possibilidades < 64) {
        p->i = i;
        p->j = j;
        possibilidades++;
        resolve(m, i, j - 1, p + 1);// tentar ir para a esquerda
    }
